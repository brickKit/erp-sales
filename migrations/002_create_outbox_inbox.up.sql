...
-- 任务运行时用 erp_sales_rw（SET LOCAL ROLE 切换）建未来的分区，两者
-- 不是同一身份，必须显式把 owner 转过去。
ALTER TABLE event_outbox OWNER TO erp_sales_rw;
...
ALTER TABLE event_inbox OWNER TO erp_sales_rw;

-- 写命令的幂等表（命令侧，与事件消费侧的 event_inbox 分开）。
-- CreateOrder/ConfirmOrder/CancelOrder/ShipOrder 的 idempotency_key
-- 落在这里去重；result_id 存该命令产生的订单 id。
