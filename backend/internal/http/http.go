...
//
// ⚠️ 全部标 besdk.Public 是阶段二的刻意状态（同 erp-finance/erp-inventory）：
// 阶段三 infra-authz 上线后要把这几条改成 assembly.yaml 里对应的真实
// 权限键（erp.sales.view/create/confirm/cancel/ship）。
package http
...
func RegisterRoutes(eng *gin.Engine, svc *service.Service) {
	...
	besdk.POST(g, "/orders", besdk.Public, createOrderHandler(svc))
	besdk.GET(g, "/orders", besdk.Public, listOrdersHandler(svc))
	besdk.GET(g, "/orders/:id", besdk.Public, getOrderHandler(svc))
	besdk.POST(g, "/orders/:id/confirm", besdk.Public, confirmOrderHandler(svc))
	besdk.POST(g, "/orders/:id/cancel", besdk.Public, cancelOrderHandler(svc))
	besdk.POST(g, "/orders/:id/ship", besdk.Public, shipOrderHandler(svc))
	besdk.POST(g, "/price/dry-run", besdk.Public, calculatePriceDryRunHandler(svc))
}
