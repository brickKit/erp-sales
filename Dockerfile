...
# ⚠️ gen/erp/finance、gen/erp/inventory、gen/mdm/customer、gen/mdm/product
# 这四份 vendored-contract 只读镜像都是本仓库自己嵌套的 go module，go.mod
# 里用本地相对路径 replace 各自指向自己（阶段四调研记录 04 §13）——go mod
# download 解析 replace 时必须已经能读到这些子目录自己的 go.mod，所以不能
# 像常见写法那样只先 COPY go.mod go.sum 再 download：本地 replace 没有
# 远程校验和可下载，必须整个源码树都在场才能解析。
COPY . .
RUN go mod download
